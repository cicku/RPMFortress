...
%description
VirtualBox is traditionally considered to be a virtualisation solution aimed at
...
sed -i 's|$Bin/docs|/usr/share/doc/%{name}-%{version}|g' %{name}
sed -i 's|$Bin/|/usr/|g' %{name} share/%{name}/*.pl
